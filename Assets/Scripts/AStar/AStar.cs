...
namespace AStar
{
    ...
    {
        public static List<Vector3Int> GetPath(Vector3Int startPosition, Vector3Int endPosition,
            bool[] obstaclesBoolArrayBool,
            MapGrid grid)
        {
            var startVertexPosition = new VertexPosition(startPosition, false);
            var endVertexPosition = new VertexPosition(endPosition, false);
        ...
        private static VertexPosition[] _FindNeighborFor(VertexPosition currentVertex, MapGrid grid,
            bool[] obstaclesBoolArrayBool)
        {
            var arrayOfNeighbors =
                new VertexPosition[VertexPosition.PossibleNeighbours.Length]; // Set size for left, right, up, down

            var arrayIndex = 0;
            foreach (var possibleNeighbor in VertexPosition.PossibleNeighbours)
            {
                var positionVector3Int = new Vector3Int(currentVertex.X + possibleNeighbor.x, 0,
                ...
                if (grid.IsCellValid(positionVector3Int.x, positionVector3Int.z))
                {
                    ...
                    arrayOfNeighbors[arrayIndex] = new VertexPosition(positionVector3Int,
                        obstaclesBoolArrayBool[grid.CalculateIndexFromVector3Int(positionVector3Int)]);
                    arrayIndex++;
                }
            }
